...
class Moorhuhn{    
    ...
    constructor() {
     ...
     rollForSide(moorhuhn,mainStage) { //if our moorhuhn did manage to reach the other side of the screen, we first roll for the side the moorhuhn has to spawn at. Depending on that we either negate the velocity, example: spawn at x = 500 -> moves to x = 0 -> v *= (-1)
        ...
        if(side === 0){ //from 0 to +n -> we turn the velocity into a positive number
            ...
            moorhuhn.x = mainStage.bgLength - 20;
            moorhuhn.velocity *= -1;
        }
...
moorhuhn.reSpawn(moorhuhn,mainStage);
